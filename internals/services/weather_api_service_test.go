...
const (
	weatherAPIBody = `{ 
		"location": { 
			...
			"localtime": "2024-05-24 16:25" 
		}, 
		"current": {
			"last_updated_epoch": 1716578100, 
			"last_updated": "2024-05-24 16:15", 
			"temp_c": 21, 
			"temp_f": 69.8, 
			"is_day": 1, 
			"condition": { 
...
func TestGetWeatherByCity(t *testing.T) {
	...
	assert.Equal("2024-05-24 16:25", result.Location.Localtime)

	assert.Equal(int(1716578100), result.Current.LastUpdatedEpoch)
	assert.Equal("2024-05-24 16:15", result.Current.LastUpdated)
