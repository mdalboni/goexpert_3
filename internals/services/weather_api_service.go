...
func NewWeatherAPIService() WeatherService {
	return &WeatherAPIService{BaseHttpService{Client: &http.Client{}}}
}
...
func (w *WeatherAPIService) GetWeatherByCity(city string) (*WeatherAPIResponse, error) {
	...
	if err != nil {
		slog.Error("Error getting weather: ", err)
		return nil, err
	}
	defer resp.Body.Close()
	if resp.StatusCode != 200 {
		slog.Error(fmt.Sprintf("Error getting weather: statusCode:%d Response:%s", resp.StatusCode, resp.Body))
